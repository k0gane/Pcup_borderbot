...
def make_json(chara_id, border, border_b60, border_b24):#json整形部
    ...
    return {"1位":{'name':border[s][0]["nickname"], 'fan_num':border[s][0]["score"], 'dif_60':culc_diff(border[s][0]["score"], border_b60[s][0]["score"]), 'dif_24':culc_diff(border[s][0]["score"], border_b24[s][0]["score"])}, 
                   ...
                   "3000位":{'name':border[s][2999]["nickname"], 'fan_num':border[s][2999]["score"], 'dif_60':culc_diff(border[s][2999]["score"], border_b60[s][2999]["score"]), 'dif_24':culc_diff(border[s][2999]["score"], border_b24[s][1099]["score"])}
                   }
...
def tweet_picture(now_time):#メイン
    ...
    text = str(nowtime.month) + "/" +  str(nowtime.day) + " " + str(nowtime.hour) + ":30 現在\nPカップボーダー\n"
    text += no1 + no2 + no3
    text += "\n#Pカップボーダー"
    print(text)
